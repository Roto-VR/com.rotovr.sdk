...
namespace driver
{
	...
	class MotionCompensationManager
	{
		...
		MMFstruct_Mover_v1* Data_RigPose = nullptr;
	...
	public:
		...
		void setOffsets(MMFstruct_Mover_v1* mover);
	...
	private:
		...
		vr::HmdVector3d_t ToEulerAngles(const vr::HmdQuaternion_t q) {
			vr::HmdVector3d_t angles;    //yaw pitch roll
			const auto q1 = q.x;
			const auto q2 = q.y;
			const auto q3 = q.z;
			const auto q0 = q.w;
			...
			//auto _x = std::atan2(sinr_cosp, cosr_cosp);
			...
			//if (std::abs(sinp) >= 1)
			//	_y = std::copysign(3.14 / 2, sinp); // use 90 degrees if out of range
			//else
			//	_y = std::asin(sinp);
			...
			//auto _z = std::atan2(siny_cosp, cosy_cosp);
			return { roll / 0.01745329251994329 ,pitch / 0.01745329251994329 ,yaw / 0.01745329251994329 };
		}
		...
		vr::HmdQuaternion_t _ChairRotation = { 1, 0, 0, 0 };

		vr::HmdVector3d_t _RefRotVel = { 0, 0, 0 };
		vr::HmdVector3d_t _RefRotAcc = { 0, 0, 0 };

		vr::HmdQuaternion_t _ZeroChairRot = { 1, 0, 0, 0 };
		vr::HmdQuaternion_t _RefChairRot = { 1, 0, 0, 0 };
		vr::HmdQuaternion_t _RefChairRotInv = { 1, 0, 0, 0 };
