...
namespace driver
{
	...
	void ServerDriver::hooksTrackedDeviceActivated(void* serverDriver, int version, uint32_t unObjectId)
	{
		...
		if (i != _deviceManipulationHandles.end())
		{
			...
			if (unObjectId == 0) {
				handle->setMotionCompensationDeviceMode(MotionCompensationDeviceMode::MotionCompensated);
			}
			else if (unObjectId == 1) {
				handle->setMotionCompensationDeviceMode(MotionCompensationDeviceMode::ReferenceTracker);
				m_motionCompensation.setMotionCompensationMode(MotionCompensationMode::ReferenceTracker, 0, 1);
			}
